	...
	class ElementManager{
		...
		function remove($aContextual){
			...
			if(($following = $this->getFollowing($aContextual))>0){
				...
				$removeCount = $this->countAllElement($aContextual);
				$this->elementList[$this->elementList[$aContextual - 1]->previous -1]->following = 0;
				array_splice($this->elementList,$aContextual - 1, $removeCount);
			}
		}
		...
		function countAllElement($aPosition){
			$r = $this->elementList[$aPosition - 1];
			//echo $this->elementList[$aPosition - 1]->name;
			$this->cAE++;
			if(count($r->children)>0) foreach($r->children as $child) $this->countAllElement($child);
			return $this->cAE;
		}
			...
			if(isset($aAddition['attribute'])){
				if(count($aAddition['attribute']) > 0){
					foreach($aAddition['attribute'] as $position => $name){
						//echo $name;
						if(isset($name['name'])){
							$att = new Attribute($name['name'], $name['value']);
							$this->elementList[$aPosition + $position - 2]->addAttribute($att);
						}
						elseif(is_array($name[0])){
							foreach($name as $n){
								//echo $n;
								$att = new Attribute($n['name'], $n['value']);
								$this->elementList[$aPosition + $position - 2]->addAttribute($att);
							}
						}
					}
